...
5. 양쪽 축 정렬 완료 되면 집게를 내려(lower) 공을 잡은 후(grip close) 다시 집게를 올리세요(raise_claw)
6. raise_claw 완료 후, 카메라를 초기 각도(180도)로 돌려 공이 집게에 잡혀 있다면 done을 호출하세요. 공이 바닥에 있으면 done이 거부됩니다.
...
- 예시: "목표: 빨간공 | 180도 좌우정렬 완료 | 90도 회전 후 좌우정렬 필요"
...
- 집게가 닫힌 상태에서는 공을 집을 수 없습니다. 공 잡기가 실패하여 재 시도 할 경우 grip open으로 집게를 열어야 합니다.
"""
